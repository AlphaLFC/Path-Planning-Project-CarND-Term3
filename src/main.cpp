          ...
          if (s != "") {
            ...
            if (event == "telemetry") {
              ...
              for (int i=0; i<sensor_fusion.size(); i++) {
                ...
                double min_safe_distance_behind = 5.0;

                // check current lane if front car is too close
                if (d < (2+4*lane+2) && d > (2+4*lane-2)) {
                  if ((check_car_s > car_s) && ((check_car_s-car_s) < ref_vel * 0.6)) {
                    KL_cost += exp(-(check_car_s-car_s));
                  }
                }

                // check left lane if it is safe to change left
                int left_lane = lane - 1;
                if (left_lane < 0) {
                  LC_cost += 100;
                } else if (d < (2+4*left_lane+2) && d > (2+4*left_lane-2)) {
                  if ((check_car_s-car_s) < ref_vel * 0.8 && (car_s-check_car_s) < min_safe_distance_behind) {
                    LC_cost += exp(-abs(check_car_s-car_s));
                  }
                }

                // check right lane if it is safe to change right
                int right_lane = lane + 1;
                if (right_lane > 2) {
                  RC_cost += 100;
                } else if (d < (2+4*right_lane+2) && d > (2+4*right_lane-2)) {
                  if ((check_car_s-car_s) < ref_vel * 0.8 && (car_s-check_car_s) < min_safe_distance_behind) {
                    RC_cost += exp(-abs(check_car_s-car_s));
                  }
                }
              ...
              if (KL_cost > 0) {
                ...
                string outstr;
                if (LC_cost < KL_cost) {
                  outstr = "left change will be safe!";
                } else if (LC_cost >= KL_cost) {
                  outstr = "left change is NOT  safe!";
                  if (LC_cost >= 100) {
                    outstr = "left change is FORBIDDEN!";
                  }
                }
                if (RC_cost < KL_cost) {
                  outstr += " right change will be safe!";
                } else if (RC_cost >= KL_cost) {
                  if (RC_cost >= 100) {
                    outstr += " right change is FORBIDDEN!";
                  } else {
                    outstr += " right change is NOT  safe!";
                  }
                }

                cout << outstr << endl;

                if (LC_cost < KL_cost && LC_cost <= RC_cost) {
                  cout << " LEFT CHANGE" << endl;
                  lane -= 1;
                } else if (RC_cost < KL_cost && RC_cost <= LC_cost) {
